...
public class MybatisGenerator {
    ...
    public static void main(String[] args) {
        ...
        if (!arguments.containsKey(CONFIG_FILE)) {
            try {
                configurationFile = new File(MybatisGenerator.class.getResource(DEFAULT_CONFIG_FILE).toURI());
        ...
        if (!configurationFile.exists()) {
            writeLine(getString("RuntimeError.1", configurationFile.getAbsolutePath()));
            return;
        }
